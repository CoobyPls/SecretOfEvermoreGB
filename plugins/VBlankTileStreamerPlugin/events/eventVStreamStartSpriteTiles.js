...
export const fields = [
  ...
  },
  {
    ...
    description: "0 means use reserved/source size. Normal effects should be 20-24 tiles. Normal hard cap is 32.",
    type: "number",
    min: 0,
    max: 64,
    defaultValue: 24,
  },
  {
    ...
    description: "How many tiles are safe in the target actor's fixed window. 0 means use the actor's reserve_tiles value.",
    type: "number",
    min: 0,
    max: 64,
    defaultValue: 24,
...
export const compile = (input, helpers) => {
  ...
  const tileCount = clamp(input.tileCount, 24, 0, experimental ? 48 : 32);
  const reservedTileCount = clamp(input.reservedTileCount, 24, 0, 64);
